// /api/chat.js — v3 (extraction depuis la page: JSON-LD + meta + regex) — CommonJS + non-streaming
...
module.exports = async (req, res) => {
  try {
    ...
    // 2) Nettoyage + extraction multi-stratégies
    ...
    const key = process.env.OPENAI_API_KEY; if (!key) return res.status(500).send('Missing OPENAI_API_KEY');
    ...
    if (!r.ok) { const t = await r.text(); return res.status(500).send(`OpenAI error: ${r.status} ${t}`); }
...
function extractJsonLd(html){
  ...
  while ((m = re.exec(html)) !== null) {
    try { const raw = m[1].trim().replace(/\/\/.*$/gm,''); const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) parsed.forEach(x => out.push(x)); else out.push(parsed);
    } catch(_) {}
  }
  return out;
}
function extractMeta(html){
  ...
  return { title: take('og:title')||take('twitter:title'),
